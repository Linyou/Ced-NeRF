...
def build_hash_encoder_kernel(
    ...
    def hash_encoder_backward_kernel(
            ...
            input_grad: ti.types.ndarray(vec3),
            B: ti.i32,
        ):
        # get hash table embedding
        ti.loop_config(block_dim=block_dim)
        for i, level in ti.ndrange(B, hash_level):
            ...
            input_grad_temp = vec3(0.0)

            for idx in ti.static(range(8)):
                w = 1.
                pos_grid_local = uvec3(0)
                dw_dpos_temp = vec3(0.0)
                for d in ti.static(range(3)):
                    if (idx & (1 << d)) == 0:
                        pos_grid_local[d] = pos_grid[d]
                        w *= 1 - pos[d]
                        # if dL/dx, then drop x, = -yz * dL/dw
                        dw_dpos_temp[d] = -(1 - pos[d])
                    else:
                        pos_grid_local[d] = pos_grid[d] + 1
                        w *= pos[d]
                        dw_dpos_temp[d] = pos[d]
                ...
                if grad_dy_temp.any():
                    table_feat = table[index_table]
                    dw_dpos = w / dw_dpos_temp
                    input_grad_temp += (table_feat * grad_dy_temp).sum() * dw_dpos
                    w_grad_dy_temp = w * grad_dy_temp
                    if w_grad_dy_temp.any():
                        hash_grad[index_table] += w_grad_dy_temp
                        
            if input_grad_temp.any():
                input_grad[i] += input_grad_temp
    ...
    def __init__(
        ...
        class _module_function(torch.autograd.Function):
            ...
            def backward(ctx, doutput):
                ...
                input_grad = torch.empty_like(input_pos)
                self._hash_encoder_backward_kernel(
                    input_pos,
                    params,
                    self.hash_map_sizes,
