...
from taichi.math import uvec3, vec3, vec4
...
def build_hash_encoder_kernel(
    ...
    out_vec = ti.types.vector(
        n=2,
        dtype=data_type,
    )
    @ti.kernel
    def hash_encoder_kernel(
            xyzs: ti.types.ndarray(vec4), 
            table: ti.types.ndarray(feat_vec),
            output_embedding: ti.types.ndarray(out_vec), 
        ...
        for i, level in ti.ndrange(B, hash_level):
            ...
            local_features = out_vec(0.)

            t_scaled = t * 3

            # Index of the lower bound for interpolation
            idx_lower = ti.floor(t_scaled)

            # Fractional part used for interpolation
            t_frac = t_scaled - idx_lower

            # Handling the case where t = 1
            idx_lower = ti.min(idx_lower, 2)

            idx_lower_scale = ti.i32(idx_lower*2)
            idx_upper_scale = ti.i32(idx_lower+1)*2
            ...
            for idx in ti.static(range(8)):
                ...
                table_vec = table[index_table]
                table_vec_lower = out_vec(0.)
                table_vec_upper = out_vec(0.)
                for ii_d in ti.static(range(2)):
                    table_vec_lower[ii_d] = table_vec[idx_lower_scale+ii_d]
                    table_vec_upper[ii_d] = table_vec[idx_upper_scale+ii_d]

                local_table_vec = table_vec_lower*(1-t_frac) + table_vec_upper*t_frac

                local_features += ti.cast(
                    w * local_table_vec, 
                    data_type
                )
    ...
    def hash_encoder_backward_kernel(
            ...
            hash_map_sizes: ti.types.ndarray(), 
            offsets: ti.types.ndarray(), 
            output_grad: ti.types.ndarray(out_vec),
            ...
            for idx in ti.static(range(8)):
                ...
                if grad_dy_temp.any():
                    ...
                    if w_grad_dy_temp.any():
                        hash_grad_temp = feat_vec(0.)
                        for ii_d in ti.static(range(2)):
                            hash_grad_temp[idx_lower_scale+ii_d] = \
                                w_grad_dy_temp[ii_d]*(1-t_frac)
                            hash_grad_temp[idx_upper_scale+ii_d] = \
                                w_grad_dy_temp[ii_d]*t_frac

                        hash_grad[index_table] += hash_grad_temp
